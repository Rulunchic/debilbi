...
function IncomingCall({ dm, info, onIgnore, onAnswer, onReject }: IncomingCallProps) {
  ...
  useMediaOutputSink(getAudioRef, preferredAudioOutputDeviceId);
...
export function CallEmbedProvider({ children }: CallEmbedProviderProps) {
  ...
  const [preferredAudioOutputDeviceId] = useSetting(settingsAtom, 'preferredAudioOutputDeviceId');
  const [callDocument, setCallDocument] = useState<Document | undefined>();
  ...
  useEffect(() => {
    ...
    if (!iframe) {
      setCallDocument(undefined);
      return undefined;
    }

    const updateDocument = () => {
      setCallDocument(iframe.contentDocument ?? undefined);
    };

    updateDocument();
    iframe.addEventListener('load', updateDocument);
    return () => {
      iframe.removeEventListener('load', updateDocument);
    };
  }, [callEmbed]);

  useDocumentAudioOutputSink(callDocument, preferredAudioOutputDeviceId);
